...
Domain/M2 определяют publication, default selection, soft delete, immutable graph и optional hook semantics. Browser UI пока показывает mock mutable config и fixed stage names; UI0.2 подтверждает точные views/commands до формы.
...
аудируемыми командами. PipelineVersion всегда read-only после publication; UI
объясняет изменение графа действием «создать следующую version».
...
- read-only stage graph и contract inspector;
