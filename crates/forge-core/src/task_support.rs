...
use forge_storage::{StorageTransaction, StoredTask, TaskPersistence};
use serde_json::json;
...
pub(crate) async fn load_scoped_task(
    transaction: &mut StorageTransaction<'_>,
    project: &Project,
    task_id: TaskId,
) -> Result<StoredTask, CoreError> {
...
pub(crate) fn require_task_revision(task: &Task, expected: u64) -> Result<(), CoreError> {
...
pub(crate) async fn pinned_pipeline_version(
    transaction: &mut StorageTransaction<'_>,
    project: &Project,
    task: &Task,
) -> Result<PipelineVersion, CoreError> {
...
pub(crate) fn current_stage_executor(
    version: &PipelineVersion,
    task: &Task,
) -> Result<ExecutorKind, CoreError> {
    let stage_id = task.current_stage_id().ok_or(CoreError::InvalidTransport {
...
pub(crate) fn stage_wait_for_current_task(
    task: &Task,
...
pub(crate) fn retained_persistence(
    project: &Project,
    task: &Task,
...
pub(crate) async fn persist_task_and_project(
    transaction: &mut StorageTransaction<'_>,
    project: &mut Project,
    task: &Task,
...
pub(crate) fn stage_payload(task: &Task) -> serde_json::Map<String, serde_json::Value> {
    event_payload([
        (
            "stage_id",
...
pub(crate) fn reject_unimplemented_system_stage(
    executor_kind: ExecutorKind,
) -> Result<(), CoreError> {
