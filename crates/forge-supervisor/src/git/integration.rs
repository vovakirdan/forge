...
impl GitBackend {
    /// Prepares a two-parent merge whose tree exactly equals the accepted candidate.
    ///
    ...
    pub async fn prepare_integration(
        ...
        let expected_target = self
            .resolve_commit(request.target, request.branch.as_str())
            .await?;
        if expected_target == request.candidate.commit {
        ...
        let repository = self
            ...
            .await?;
        self.fetch_object(&repository, request.target, &expected_target)
            .await?;
        self.fetch_object(
            &repository,
        ...
        self.ensure_supported_candidate(&repository, request.candidate)
            .await?;
        if !self
            .ancestor(&repository, &expected_target, &request.candidate.commit)
            .await?
        {
        ...
        let merge_commit = GitObjectId::new(
    ...
    pub async fn reconcile_integration(
        ...
        let Some(current) = self.read_ref(target, intent.target_ref.as_str()).await? else {
            return Ok(GitIntegrationState::Unknown);
        };
        ...
        if current == intent.expected_target {
